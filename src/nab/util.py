...
def byteify(input):
    '''used to correctly load .json files
...
def merge_two_dicts(x, y):
    '''https://stackoverflow.com/a/26853961/6028830'''
    z = x.copy()   # start with x's keys and values
    z.update(y)    # modifies z with y's keys and values & returns None
    return z
    
def getGridParams(D, DA, detectorName, detectorConstructor, paramNames=None, paramValues=None):
    '''generates multiple detectors in order to finetune parameters'''
    
    detectors = {}
    detectorArgs = {}
    
    if paramNames is not None and paramValues is not None:
        for pn in paramNames:
            for pv in paramValues:
                genName = detectorName+'_'+pn+'_'+str(pv)
                detectors[genName]=detectorConstructor
                detectorArgDict = {}
                detectorArgDict[pn]=pv
                detectorArgs[genName]=detectorArgDict
    else:
        detectors[detectorName] = detectorConstructor
        detectorArgs[detectorName] = {}
            
    D = merge_two_dicts(D,detectors)
    DA = merge_two_dicts(DA,detectorArgs)
